    ...
    html += '<div class="space-y-2">' + recs.map(function(r){
      return '<div class="flex items-center justify-between rounded-xl border bg-white p-4" style="border-color:#E8D4DB">' +
        ...
        '<div class="flex items-center gap-1">' +
          ...
          '<button data-view-medical-history data-student-id="' + esc(r.studentId || '') + '" data-student-name="' + esc(r.studentName || '') + '" class="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style="color:#7A7A7A" title="View Medical History" onmouseenter="this.style.color=\'#2563EB\'" onmouseleave="this.style.color=\'#7A7A7A\'">' + icon('history', 13) + '</button>' + '<button data-add-medical-history="' + esc(r.id) + '" data-student-id="' + esc(r.studentId || '') + '" data-student-name="' + esc(r.studentName || '') + '" data-department="' + esc(r.department || '') + '" data-year-level="' + esc(r.yearLevel || '') + '" data-section="' + esc(r.section || '') + '" data-blood-type="' + esc(r.bloodType || '') + '" data-course="' + esc(r.course || '') + '" class="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style="color:#7A7A7A" title="Add Medical History" onmouseenter="this.style.color=\'#8B5CF6\'" onmouseleave="this.style.color=\'#7A7A7A\'">' + icon('plus', 13) + '</button>' +
...
function renderCrudModule(key) {
  ...
  if (ms.loading) {
    ...
    var emptyHint = key === 'students'
      ...
      : key === 'medicalHistory'
        ? 'No records found. Open Medical Records and use the + button on a student\'s card to add one.'
